...
/*   Updated: 2026/04/01 21:33:06 by vloureir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void)
{
	...
	// HOW TO PROTECT FROM SIGSEGV
	rrf = zz.makeForm("nothing", "Zuzuzu");
	rrf = zz.makeForm("shrubbery request", "Zuzuzu");
	rrf = zz.makeForm("robotomy request", "Zuzuzu");
	if (rrf != NULL)
	{
		c.executeForm(*rrf);
		c.signForm(*rrf);
